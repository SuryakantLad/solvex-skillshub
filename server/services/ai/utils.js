...
export function serializeCandidate(emp) {
  return {
    ...
    skills: (emp.skills ?? []).filter((s) => !s.inferred).slice(0, 20).map((s) => `${s.name} (${s.proficiency}, ${s.yearsOfExperience || 0}y)`),
...
class LRUCache {
  ...
  set(key, value) {
    ...
    if (this._map.size >= this._maxSize) this._map.delete(this._map.keys().next().value);
...
export const searchCache = new LRUCache(50, 2 * 60 * 1000);
export const teamCache = new LRUCache(20, 5 * 60 * 1000);
