...
const QUICK_ACTIONS = [
  {
    ...
    desc: 'Find talent with natural language',
    gradient: 'from-violet-500 to-indigo-500',
    glow: 'hover:shadow-[0_4px_20px_rgba(124,58,237,0.15)]',
  },
  {
    href: '/hr/directory',
    icon: Users,
    label: 'Employee Directory',
    desc: 'Browse all talent profiles',
  ...
  },
  {
    ...
    icon: Wrench,
    label: 'Team Builder',
    desc: 'AI assembles optimal teams',
    gradient: 'from-emerald-500 to-teal-500',
    glow: 'hover:shadow-[0_4px_20px_rgba(16,185,129,0.15)]',
  },
];

export default function HRDashboardClient({ stats, topSkills, departments, recentEmployees }) {
  const { user } = useAuth();
    ...
    <div className="space-y-8">
      ...
      >
        <div className="absolute inset-0 opacity-30"
          style={{
            backgroundImage: 'radial-gradient(circle at 80% 50%, hsl(var(--primary)/0.15), transparent 60%)',
          }}
        />
        <div className="relative flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <Link href="/hr/search" className="sm:shrink-0">
          ...
          <Card className="h-full">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Link href="/hr/directory" className="text-xs text-primary hover:underline underline-offset-2">
                  View all →
                </Link>
              </div>
            </CardHeader>
            <CardContent className="space-y-1">
              {recentEmployees.slice(0, 5).map((emp) => (
                <Link key={emp._id} href={`/hr/directory/${emp._id}`}>
