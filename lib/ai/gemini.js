/**
 * lib/ai/gemini.js
 * Production-grade Claude AI client for TalentGraph AI.
 *
 * Features:
...
/**
 * Single Claude AI call — no retry.
 * @param {string} prompt
...
async function callGemini(prompt, generationConfig) {
  ...
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Claude AI request timed out after 55s')), HARD_TIMEOUT_MS)
  );
...
async function _withRetry(prompt, config, maxRetries) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      _log('info', `Claude AI OK`, { attempt, ms: Date.now() - t0, chars: text.length });
      return text;
    } catch (error) {
      lastError = error;
      ...
      _log('warn', `Claude AI attempt ${attempt}/${maxRetries} failed`, {
        error: error.message,
        ms,
        isRateLimit,
        isOverloaded,
