/**
 * lib/ai/extractor.js
 * Resume extraction pipeline using Gemini 2.5 Flash.
 * Replaces the previous Claude-based extractor.
 *
 * External interface is unchanged — callers use extractResumeData().
 */
...
// ─── JSON repair (kept for backward-compat imports) ───────────────────────────
...
/**
 * Extract structured resume data with Gemini, retry, validation, inference, and fallback.
 *
 * @param {string} resumeText - Raw text extracted from PDF
 * @param {object} [options]
 * @param {number} [options.maxRetries=3]
 * @param {boolean} [options.enableInference=true]
 * @returns {Promise<object>} - Validated, sanitized, optionally enriched resume
 */
export async function extractResumeData(resumeText, options = {}) {
  const { maxRetries = DEFAULT_MAX_RETRIES, enableInference = true } = options;
