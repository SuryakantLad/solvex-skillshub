...
export default function StatsCard({ title, value, description, icon: Icon, trend, color = 'default' }) {
  ...
  return (
    <motion.div
      ...
      className={cn('rounded-2xl border border-border bg-card p-5 transition-all duration-200 cursor-default', cfg.border, cfg.glow)}
    >
      ...
      <div className="space-y-1">
        ...
        {trend && (
          <p className={cn('text-xs font-semibold flex items-center gap-1', trend.positive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400')}>
